...
class AdminVisaController extends Controller
{
	...
	{
		$this->config = Config::get('components.visa');
		if(Route::current()){
			View::share('menu', $menu->index(Route::current()->getUri())->render());
		}

		Breadcrumbs::register('admin.visa.index', function($breadcrumbs){
			$breadcrumbs->push('Визовая поддержка', route('admin.visa.index'));
		});
	...
	public function index()
	{
		...
		$data['data'] = Category::type('visa')->level(1)->orderBy('position', 'DESC')->with(['get_child', 'get_parent'])->paginate(30);
	...
	{
		if( !$category = Category::whereType('visa')->first()){
			Category::create(['title' => 'Новый раздел', 'url' => str_slug('Новый раздел')]);
			$category = Category::whereType('visa')->first();
		}
		$test = Request::create('/admin/visa', 'POST', [
	...
	public function show($id)
	{
		...
		$data['data'] = Visa::whereCategory($id)->paginate(30);
		View::share('validator', '');

		Breadcrumbs::register('admin.visa.category', function($breadcrumbs, $data)
		{
			$breadcrumbs->parent('admin.visa.index');
			if($find_parent = Category::find($data->parent)){
				$breadcrumbs->push($find_parent->title, route('admin.visa.show', $find_parent->id));
			...
			$breadcrumbs->push($data->title, route('admin.visa.show', $data->id));
		});

		return view('admin.visa.show', $data);
	}
	...
	{
		$data['data'] = Visa::with(['get_category', 'get_seo', 'get_templates'])->findOrFail($id);
		...
		Breadcrumbs::register('admin.visa.edit', function($breadcrumbs, $data)
		{
			$breadcrumbs->parent('admin.visa.index');
			//dd($data);
			...
			if(isset($find_parent_3->title)){
				$breadcrumbs->push($find_parent_3->title, '/admin/visa/'. $find_parent_3->id);
			}
			if(isset($find_parent_2->title)){
				$breadcrumbs->push($find_parent_2->title, '/admin/visa/'. $find_parent_2->id);
			}
			if(isset($find_parent->title)){
				$breadcrumbs->push($find_parent->title, '/admin/visa/'. $find_parent->id);
			}

			$breadcrumbs->push($data->title, '/admin/visa/'. $data->id);
		});
