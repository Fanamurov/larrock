...
@section('content')
    <table class="table">
        ...
        <tr>
            ...
            <td><a href="/feed/{{ $category->url}}">/feed/{{ $category->url}}</a></td>
            <td class="row-updated_at">{{ $category->updated_at }}</td>
            ...
            <td class="row-active">
                <div class="btn-group pull-right btn-group_switch_ajax" role="group">
                    <button type="button" class="btn btn-xs btn-primary @if($category->active === 0) btn-outline @endif"
                            data-row_where="id" data-value_where="{{ $category->id }}" data-table="category"
                            data-row="active" data-value="1">on</button>
                    <button type="button" class="btn btn-xs btn-warning @if($category->active === 1) btn-outline @endif"
                            data-row_where="id" data-value_where="{{ $category->id }}" data-table="category"
                            data-row="active" data-value="0">off</button>
                </div>
            </td>
            <td class="row-edit" title="Чем больше, тем выше в списках">
                <a href="/admin/category/{{ $category->id }}/edit" class="btn btn-block btn-primary btn-xs"><i class="fa fa-pencil"></i> Изменить</a>
            </td>
            <td class="row-delete">
