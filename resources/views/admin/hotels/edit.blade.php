...
@section('content')
    ...
    <div class="ibox float-e-margins">
        <div class="ibox-title background-transparent">
            {!! Breadcrumbs::render('admin.hotels.edit', $data) !!}
                ...
                @foreach($data->get_category as $category)
                    <ul>
                        @if($category->get_parent()->first()->id === 377)
                            <li>{{ $category->title }}: <a href="/tours/vidy-otdykha/{{ $category->url }}">/tours/vidy-otdykha/{{ $category->url }}</a></li>
                        @else
                            <li>{{ $category->title }}: <a href="/{{ $app['name'] }}/{{ $data->url }}">/{{ $app['name'] }}/{{ $data->url }}</a></li>
                        @endif
                    </ul>
                @endforeach
            @endif
